...
class DualBranchGPM(nn.Module):
    """
    Dual-branch GPM used by DeAOT.

    IMPORTANT:
    - Internally we can fuse branches with a gate to stabilize training / improve blending.
    - But externally, DeAOT's decoder path expects a 2*d_model feature tensor:
        cat_output = concat([visual_branch, id_branch], dim=2)
      (e.g., 512 channels when d_model=256). If we return only d_model, you'll hit:
        expected 512 channels, got 256
    """
    def __init__(self,
                 num_layers=2,
                 d_model=256,
        ...
        # --- core GPM layers (unchanged) ---
        block = GatedPropagationModule
        layers = []
        for idx in range(num_layers):
            if droppath_scaling:
                droppath_rate = 0 if num_layers == 1 else droppath * idx / (num_layers - 1)
            else:
                droppath_rate = droppath
        ...
        # --- gated fusion: takes [x_vis, x_id] -> gate -> blended x_vis ---
        # gate in [0,1], shape [T,B,d_model]
        self.gate_proj = nn.Linear(d_model * 2, d_model)

        # Norms must match what we RETURN (cat_output = 2*d_model)
        out_dim = d_model * 2
        num_norms = num_layers - 1 if intermediate_norm else 0
        if final_norm:
            num_norms += 1
        self.decoder_norms = [_get_norm(out_dim, type='ln') for _ in range(num_norms)] if num_norms > 0 else None
        if self.decoder_norms is not None:
            self.decoder_norms = nn.ModuleList(self.decoder_norms)
    ...
    def forward(self,
        ...
        # visual branch token stream (T,B,d_model)
        output = self.emb_dropout(tgt)

        intermediate = []
        intermediate_memories = []
        output_id = None  # id branch stream (T,B,d_model)
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # ---- gated fusion (only modifies the "visual" stream we propagate) ----
            # output/output_id: [T,B,d_model]
            cat = torch.cat([output, output_id], dim=2)          # [T,B,2*d_model]
            gate = torch.sigmoid(self.gate_proj(cat))            # [T,B,d_model]
            fused_vis = gate * output + (1.0 - gate) * output_id # [T,B,d_model]

            # propagate fused visual features to next layer
            output = fused_vis

            # IMPORTANT: return cat_output for downstream decoder (2*d_model)
            cat_output = torch.cat([output, output_id], dim=2)   # [T,B,2*d_model]

            if self.return_intermediate:
                intermediate.append(cat_output)
                intermediate_memories.append(memories)

        # Apply norms to cat_output (2*d_model)
        if self.decoder_norms is not None:
            if self.final_norm:
            ...
            if self.return_intermediate:
                ...
                if self.intermediate_norm:
                    for i in range(len(intermediate) - 1):
                        intermediate[i] = self.decoder_norms[i](intermediate[i])
